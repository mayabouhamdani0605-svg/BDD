-- ============================================================
-- BOUTIQUE PC EN LIGNE — Base de données finale
-- INFOB212 — Bases de données — Groupe 6
-- Hiba Khadiri, Maya Bouhamdani, Yahya Lahniti, Ahmed Feki
-- ============================================================
...
-- (CLIENT et MEMBRE_COMPTABILITE en dépendent)
-- ============================================================
...
CREATE TABLE UTILISATEUR (
    ...
    -- Note : mots de passe stockés en clair dans ce projet académique.
    -- En production, il faudrait utiliser un algorithme de hachage (bcrypt, SHA-256…).
);

-- ============================================================
-- ÉTAPE 2 : COMPOSANTS TECHNIQUES
-- (PROCESSEUR, MEMOIRE_RAM, CARTE_GRAPHIQUE, ECRAN)
-- ============================================================
...
CREATE INDEX idx_gpu_modele         ON CARTE_GRAPHIQUE(modele);
...
BEFORE INSERT ON PROCESSEUR FOR EACH ROW
BEGIN
    IF NEW.vitesse_ghz <= 0 THEN
        SIGNAL SQLSTATE '45000'
        SET MESSAGE_TEXT = 'Erreur : la vitesse du processeur doit etre superieure a 0 GHz.';
    END IF;
...
BEFORE UPDATE ON PROCESSEUR FOR EACH ROW
BEGIN
    IF NEW.vitesse_ghz <= 0 THEN
        SIGNAL SQLSTATE '45000'
        SET MESSAGE_TEXT = 'Erreur : la vitesse du processeur doit etre superieure a 0 GHz.';
...
BEFORE INSERT ON ECRAN FOR EACH ROW
BEGIN
    IF NEW.diagonale_pouces < 10 OR NEW.diagonale_pouces > 20 THEN
        SIGNAL SQLSTATE '45000'
        SET MESSAGE_TEXT = 'Erreur : la diagonale doit etre comprise entre 10 et 20 pouces.';
    END IF;
END$$
...
INSERT INTO MEMOIRE_RAM (capacite_gb) VALUES (8), (16), (32), (64);
...
BEGIN
    IF NEW.prix_vente <= 0 THEN
        SIGNAL SQLSTATE '45000'
        SET MESSAGE_TEXT = 'Erreur : le prix de vente ne peut pas etre nul ou negatif.';
    END IF;
END$$
...
-- Fiche technique complète d'un produit (jointure 5 tables évitée à chaque requête)
...
-- Catalogue trié par prix croissant (utile pour les clients à budget limité)
...
-- Alerte de stock : estime le nombre de jours avant rupture
-- en divisant le stock actuel par la cadence de vente des 90 derniers jours
...
INSERT INTO PRODUIT (ref_produit, nom_commercial, marque, prix_vente, etat, disponibilite,
                     stock_quantite, systeme_exploitation, poids_kg, description,
                     id_categorie, id_processeur, id_ram, id_gpu, id_ecran) VALUES
...
-- ============================================================
-- ÉTAPE 4 : CLIENT, COMMANDE, LIGNE_COMMANDE, AVIS
-- ============================================================
...
CREATE TABLE LIGNE_COMMANDE (
    ...
    CONSTRAINT FK_LC_PRODUIT  FOREIGN KEY (id_produit)  REFERENCES PRODUIT(id_produit)    ON DELETE RESTRICT
);
...
CREATE TABLE AVIS (
    ...
    CONSTRAINT FK_AVIS_CLIENT       FOREIGN KEY (id_client)  REFERENCES CLIENT(id_client)   ON DELETE SET NULL,
    CONSTRAINT FK_AVIS_PRODUIT      FOREIGN KEY (id_produit) REFERENCES PRODUIT(id_produit) ON DELETE RESTRICT,
...
-- ============================================================
-- TRIGGERS — Client, Commande, Avis
-- ============================================================

DELIMITER $$

-- Anonymisation RGPD : met id_client à NULL dans COMMANDE et AVIS avant suppression du CLIENT
...
-- Vérifie qu'un avis ne peut être posté que si le client a une commande livrée pour ce produit
...
-- Recalcule automatiquement le prix_total de la commande après chaque insertion dans LIGNE_COMMANDE
CREATE TRIGGER TRG_MAJ_MONTANT_COMMANDE_INSERT
AFTER INSERT ON LIGNE_COMMANDE FOR EACH ROW
BEGIN
    UPDATE COMMANDE
    SET prix_total = (
...
-- ============================================================
-- VUES — Clients et Produits
-- ============================================================

-- Profil de fidélité de chaque client : commandes, avis, dépenses, statut VIP/Premium/Standard
...
-- Statistiques de vente et de satisfaction par produit
...
-- ============================================================
-- DONNÉES — Clients, Commandes, Avis
-- ============================================================
...
-- (MEMBRE_COMPTABILITE, RAPPORT_FINANCIER, RAPPORT_MENSUEL)
-- ============================================================
...
CREATE INDEX idx_rapport_annee ON RAPPORT_FINANCIER(annee);

-- ============================================================
-- VUES — Comptabilité
-- (colonne nommée chiffreAffaires pour cohérence avec comptabilite.py)
-- ============================================================

-- Chiffre d'affaires total par année (commandes livrées uniquement)
CREATE VIEW VUE_CA_ANNUEL AS
SELECT
    YEAR(c.date_commande)                      AS annee,
    SUM(l.quantite * l.prix_unitaire_capture)  AS chiffreAffaires
FROM COMMANDE c
JOIN LIGNE_COMMANDE l ON c.id_commande = l.id_commande
WHERE c.statut = 'livree'
GROUP BY YEAR(c.date_commande);

-- Chiffre d'affaires par mois et par année (granularité mensuelle)
...
-- Met à jour automatiquement RAPPORT_FINANCIER et RAPPORT_MENSUEL
-- dès qu'une commande passe au statut 'livree'
...
BEGIN
    ...
    IF NEW.statut = 'livree' AND OLD.statut <> 'livree' THEN
        ...
        ON DUPLICATE KEY UPDATE chiffre_affaires_annuel = chiffre_affaires_annuel + total;
        ...
        ON DUPLICATE KEY UPDATE chiffre_affaires_mensuel = chiffre_affaires_mensuel + total;
    END IF;
END$$
...
-- role_comptabilite : lecture des rapports financiers uniquement
GRANT SELECT ON boutique_pc.VUE_CA_ANNUEL      TO role_comptabilite;
GRANT SELECT ON boutique_pc.VUE_CA_MENSUEL     TO role_comptabilite;
GRANT SELECT ON boutique_pc.RAPPORT_FINANCIER  TO role_comptabilite;
GRANT SELECT ON boutique_pc.RAPPORT_MENSUEL    TO role_comptabilite;

-- role_client : consultation du catalogue et gestion de son compte
GRANT SELECT ON boutique_pc.PRODUIT                 TO role_client;
GRANT SELECT ON boutique_pc.CATEGORIE               TO role_client;
GRANT SELECT ON boutique_pc.PROCESSEUR              TO role_client;
GRANT SELECT ON boutique_pc.MEMOIRE_RAM             TO role_client;
GRANT SELECT ON boutique_pc.CARTE_GRAPHIQUE         TO role_client;
GRANT SELECT ON boutique_pc.ECRAN                   TO role_client;
GRANT SELECT ON boutique_pc.VUE_COMPOSANTS_PRODUIT  TO role_client;
GRANT SELECT ON boutique_pc.VUE_PRODUITS_PAR_PRIX   TO role_client;
GRANT INSERT ON boutique_pc.UTILISATEUR             TO role_client;
GRANT INSERT, UPDATE, DELETE ON boutique_pc.CLIENT  TO role_client;
GRANT INSERT ON boutique_pc.COMMANDE                TO role_client;
GRANT INSERT ON boutique_pc.LIGNE_COMMANDE          TO role_client;
GRANT INSERT, DELETE ON boutique_pc.AVIS            TO role_client;
