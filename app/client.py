...
def connexion_client():
    ...
    try:
        cursor.execute("""
            ...
            WHERE c.email = %s AND u.mot_de_passe = %s
        """, (email, mot_de_passe))
...
def inscrire_client():
    ...
    try:
        ...
        cursor.execute(
            "INSERT INTO UTILISATEUR (email, mot_de_passe) VALUES (%s, %s)",
            (email, mot_de_passe)
        )
        id_utilisateur = cursor.lastrowid

        cursor.execute(
            "INSERT INTO CLIENT (id_utilisateur, nom, prenom, email, telephone, adresse_livraison, date_inscription) VALUES (%s, %s, %s, %s, %s, %s, CURDATE())",
            (id_utilisateur, nom, prenom, email, telephone, adresse)
        )

        conn.commit()
        print("Inscription reussie ! Vous pouvez maintenant vous connecter.")
...
def modifier_client(id_client):
    ...
    try:
        cursor.execute(
            "SELECT nom, prenom, email, telephone, adresse_livraison FROM CLIENT WHERE id_client = %s",
            (id_client,)
        )
        client = cursor.fetchone()
        ...
        cursor.execute(
            "UPDATE CLIENT SET nom = %s, prenom = %s, telephone = %s, adresse_livraison = %s WHERE id_client = %s",
            (nom, prenom, telephone, adresse, id_client)
        )
...
def desinscrire_client(id_client):
    ...
    try:
        cursor.execute(
            "SELECT id_utilisateur FROM CLIENT WHERE id_client = %s",
            (id_client,)
        )
        result = cursor.fetchone()
        if not result:
        ...
        conn.commit()
        print("Desinscription effectuee. Vos donnees personnelles ont ete supprimees.")
